...
app.get("/api/openai-usage", async (req, res) => {
  try {
    const monthlyBudget = Number(
      process.env.OPENAI_MONTHLY_BUDGET || 100
    )
    ...
    res.json({
      ...
      note:
        "OpenAI spend is currently protected by OpenAI Spend Alerts. Actual API usage integration can be added when Admin Key access is available.",
...
// =========================================================
// [10] DEV PAID TOKEN ROUTE (테스트할때사용)
// =========================================================
/*
=========================================================
...
async function sendPaidReportEmail({
    ...
    const normalizedEmailLanguage = normalizeLanguage(language || "ko")
    const downloadUrl = buildPaidReportDownloadUrl({
        token,
    ...
    const emailText =
        emailCopy[normalizedEmailLanguage] || emailCopy.en

    const subject = emailText.subject

    const html = `
        <div style="font-family:Arial,Helvetica,sans-serif;max-width:640px;margin:0 auto;padding:32px;color:#111;">
            <h1 style="font-size:24px;margin:0 0 16px;">${emailText.title}</h1>
